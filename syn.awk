BEGIN {
	...
	MAX_WIDTH = 80
	if (ENVIRON["SYN_MAX_WIDTH"] ~ /^[0-9]+$/) {
		if (int(ENVIRON["SYN_MAX_WIDTH"]) < MAX_WIDTH) {
			MAX_WIDTH = int(ENVIRON["SYN_MAX_WIDTH"])
		}
	}
...
function printverse(verse,    word_count, characters_printed) {
	if (ENVIRON["SYN_NOLINEWRAP"] != "" && ENVIRON["SYN_NOLINEWRAP"] != "0") {
		printf("%s\n", verse)
		return
	}
