#!/bin/sh
# syn: Read the Word of God from your terminal in Synodal translation
...
while [ $# -gt 0 ]; do
	...
	if [ "$1" = "--" ]; then
		...
		get_data syn.tsv | awk -v cmd=list "$(get_data syn.awk)"
		exit
	elif [ "$1" = "-W" ]; then
		export syn_NOLINEWRAP=1
		shift
...
if [ $# -eq 0 ]; then
	...
	while true; do
		...
		get_data syn.tsv | awk -v cmd=ref -v ref="$ref" "$(get_data syn.awk)" | ${PAGER}
	done
	exit 0
fi

get_data syn.tsv | awk -v cmd=ref -v ref="$*" "$(get_data syn.awk)" | ${PAGER}
